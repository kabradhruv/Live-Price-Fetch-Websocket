                            ...
                            if buy_price is not None and sell_price is not None:
                                try:
                                    ...
                                    if profit_pct >= PROFIT_THRESHOLD:
                                        ...
                                        # **Save to CSV if profit_pct > 1%**
                                        if profit_pct > PROFIT_THRESHOLD_FOR_CSV:
                                            file_exists = os.path.isfile("arbitrage_log.csv")
                                            with open("arbitrage_log.csv", mode="a", newline="") as file:
                                                writer = csv.DictWriter(file, fieldnames=opportunity.keys())
                                                if not file_exists:
